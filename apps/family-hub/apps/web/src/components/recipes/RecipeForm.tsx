...
export default function RecipeForm({ onGenerate, onGeneratePlan, loading }: RecipeFormProps) {
  ...
  const CHIP_COLORS = ['recipe-chip-coral', 'recipe-chip-mango', 'recipe-chip-mint', 'recipe-chip-sky'];
    ...
    <div className="recipe-card p-6 space-y-6">
      ...
      {mode === 'recette' && (
        <div>
          ...
          <div className="flex flex-wrap gap-2 p-3 border-2 border-orange-200 rounded-2xl min-h-[52px] focus-within:border-orange-400 transition-all bg-orange-50/30">
            {ingredients.map((item, i) => (
              <span
                key={item}
                className="flex items-center gap-1 text-sm px-3 py-1.5 rounded-full font-semibold text-white shadow-sm"
                style={{ background: ['#FF6B6B', '#FF9F43', '#6BCB77', '#4D96FF', '#9B59B6'][i % 5] }}
              >
                {item}
                <button
                  onClick={() => setIngredients(ingredients.filter((x) => x !== item))}
                  className="opacity-70 hover:opacity-100 font-bold ml-0.5"
                >×</button>
              </span>
            ))}
            <input
          ...
          {[
            ...
            <div key={label}>
              ...
              <div className="flex items-center rounded-2xl border-2 overflow-hidden" style={{ borderColor: `${color}30`, background: `${color}08` }}>
                <button onClick={() => set((n) => Math.max(min, n - 1))} className="px-4 py-3 text-xl font-light transition-colors" style={{ color }}>−</button>
                <span className="flex-1 text-center text-xl font-black text-stone-900">{value}</span>
                <button onClick={() => set((n) => Math.min(max, n + 1))} className="px-4 py-3 text-xl font-light transition-colors" style={{ color }}>+</button>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          {FILTERS.map((f, i) => (
            <button
              ...
              className={`recipe-chip ${selectedFilters.includes(f) ? CHIP_COLORS[i % CHIP_COLORS.length] : 'recipe-chip-idle'}`}
            >{f}</button>
          ))}
        </div>
      </div>
      ...
      <div>
        ...
        >
          <svg className={`w-3.5 h-3.5 transition-transform ${showAdvanced ? 'rotate-90' : ''}`} viewBox="0 0 12 12" fill="currentColor">
            <path d="M4.5 2L8.5 6L4.5 10" stroke="currentColor" strokeWidth="1.5" fill="none" strokeLinecap="round" strokeLinejoin="round"/>
          </svg>
          Options avancées
          {advancedCount > 0 && (
            <span className="bg-gradient-to-r from-orange-400 to-red-400 text-white text-xs font-bold w-5 h-5 rounded-full flex items-center justify-center">
              {advancedCount}
            </span>
          )}
        </button>
        ...
        {showAdvanced && (
          <div className="mt-4 space-y-4 pl-4 border-l-2 border-orange-200 animate-slide-down">
            <div>
              <label className="block mb-2 text-sm font-bold text-stone-600">Type de plat</label>
              <div className="flex flex-wrap gap-2">
                {PLAT_TYPES.map((p, i) => (
                  <button key={p} onClick={() => togglePlatType(p)}
                    className={`recipe-chip ${platTypes.includes(p) ? CHIP_COLORS[i % CHIP_COLORS.length] : 'recipe-chip-idle'}`}
                  >{p}</button>
                ))}
              </div>
            </div>
              ...
              <div className="flex flex-wrap gap-2">
                {CUISINES.map((c, i) => (
                  <button key={c} onClick={() => toggleCuisine(c)}
                    className={`recipe-chip ${cuisineTypes.includes(c) ? CHIP_COLORS[i % CHIP_COLORS.length] : 'recipe-chip-idle'}`}
                  >{c}</button>
                ))}
              </div>
            </div>
            <div>
              ...
              <div className="flex gap-2">
                {DIFFICULTIES.map((d, i) => (
                  <button key={d} onClick={() => setDifficulty(difficulty === d ? undefined : d)}
                    className={`recipe-chip ${difficulty === d ? CHIP_COLORS[i] : 'recipe-chip-idle'}`}
                  >{d}</button>
                ))}
              </div>
            </div>
            <div>
              ...
              <div className="flex gap-2">
                {DURATIONS.map((dur, i) => (
                  <button key={dur} onClick={() => setMaxDuration(maxDuration === dur ? undefined : dur)}
                    className={`recipe-chip ${maxDuration === dur ? CHIP_COLORS[i] : 'recipe-chip-idle'}`}
                  >{dur}</button>
                ))}
              </div>
            </div>
      ...
      >
        {loading ? (
          ...
          <><span className="text-xl">🍳</span><span>Générer la recette</span></>
        ) : (
          <><span className="text-xl">🛒</span><span>Générer le plan de repas</span></>
        )}
      </button>
    </div>
  );
